...
public class AvatarUtils {
    ...
    public static void main(String[] args) throws IOException {
        String name = "chengl";
        String path = create(name, "/Users/chenguolin/Downloads", name);
     ...
     * @throws IOException
     */
    public static String create(String nickName, String outPath, String imageName)
            throws IOException {
        int width = 100;
        int height = 100;
        int nameLen = nickName.length();
        String nameWritten;
        //如果用户输入的姓名少于等于2个字符，不用截取
        if (nameLen <= 2) {
            nameWritten = nickName;
        } else {
            //如果用户输入的姓名大于等于3个字符，截取后面两位
            String first = nickName.substring(0, 1);
            if (isChinese(first)) {
                //截取倒数两位汉字
                nameWritten = nickName.substring(nameLen - 2);
            } else {
                //截取前面的两个英文字母
                nameWritten = nickName.substring(0, 2).toUpperCase();
            }
        }

        String filename = outPath + File.separator + imageName + ".jpg";
